const myself = {
  en: [
    {
      title: "About Me",
      text: [
        "I'm Hikaro Fernandes de Carvalho, I'm 29 years old. I'm very proactive.  I like a job well done. I'm spontaneous, a bit boring sometimes, but having a little patience, I can handle it. 'HAHAHAHA'!",
      ],
    },
    {
      title: "My Experiences",
      text: [
        "In my experiences with work. I have some accomplishments such as:",
        'In my first job as a young storekeeper`s apprentice, I managed to reorganize the entire storeroom, carry out all the insertion in the system for more than 3 years of invoices that were stopped. I managed to keep it clean and well organized.“I managed to save the company with parts that I ended up finding while organizing the stock".',
        "In my second job, I performed many appointments during my temporary outsourced contract by the company Adminas for the TRE, where I managed to reduce the queue of people to be served daily, as it turned out to be one of the fastest in the biometric re-registration service for which there was a temporary contract.",
        'In my third service, where I worked as a systems` trainer, I managed to learn the 3 systems that I would have to teach without much help, spending just 1 month. "The company had 6 modules, each module corresponding to a complete application for recording, removing, updating data in addition to generating reports and graphs on the base data it dealt with. (car sales module, financial module, accounting module, sales module parts, tractor sales module)',
        "In my last job where I worked as a Computer Technician, I performed some tasks in addition to the maintenance of servers, networks, and computers, tasks such as helping lawyers in the use of Brazilian court systems and helping with graphic material such as printing, scanning, and photocopying. My achievements in the company were to streamline the service to lawyers, implement a script to clean and shut down machines via the network (the company did not have software for this and so far it was all done manually) where on the 70 machines that existed in the company that was used by lawyers during the day, had their files deleted, and the machine turned off at the end of the day. Another achievement was to seek to learn court systems that did not belong to Goiás, as there were no places to help lawyers who could help them here in Goiás.",
      ],
    },
    {
      title: "What I Don't Like in one job!",
      text: [
        "Well, in all my work, the only thing that made me sad was the lack of meritocracy. I believe that what makes a job fair is a meritocracy. Someone who is well-regarded for his effort and hard work, not for being nominated by a high-ranking person.",
      ],
    },
    {
      title: "What is the best job for me!",
      text: [
        "What I believe to be a quality job is precisely a job where the company uses meritocracy as the main idea to treat its employees.",
      ],
    },
  ],
  br: [
    {
      title: "Sobre Mim",
      text: [
        "Eu sou Hikaro Fernandes de Carvalho, tenho 29 anos. Sou muito proativo e gosto de realizar muito bem minhas tarefas. Sou muito espontâneo e meio chato as vezes. Mas tendo um pouco de paciência podem me suportar! 'kkkkkkkk'!",
      ],
    },
    {
      title: "Minhas experiências",
      text: [
        "Em minhas esperiências profissionais, possuo algumas realizações como:",
        'No meu primeiro emprego como jovem aprendiz de almoxarife, consegui reorganizar todo o almoxarifado, realizar toda a inserção de notas fiscais no sistema que por mais de 3 anos estavam paradas. Consegui mantê-lo limpo e bem organizado. “Consegui ajudar a empresa com peças que acabei encontrando enquanto organizava o estoque”.',
        "Em meu segundo emprego, realizei muitos atendimentos durante meu contrato de terceirização temporária pela empresa Adminas para o TRE, onde consegui diminuir a fila de pessoas a serem atendidas diariamente, pois acabou sendo uma das mais rápidas no quesito biometria. serviço de registro para o qual havia um contrato temporário.",
        'Em meu terceiro serviço, onde trabalhei como implantador de sistemas, consegui aprender os 3 sistemas que teria que ensinar sem muita ajuda, gastando apenas 1 mês. "A empresa possuía 6 módulos, cada módulo correspondendo a um sistema completo para registro, remoção, atualização de dados, além de gerar relatórios e gráficos sobre os dados básicos que tratava. (módulo de vendas de carros, módulo financeiro, módulo contábil, módulo de vendas de peças , módulo de vendas de tratores)',
        "Em meu último emprego onde trabalhei como Técnico de Informática, realizei algumas tarefas além da manutenção de servidores, redes e computadores, tarefas como auxiliar advogados na utilização dos sistemas judiciários brasileiros e auxiliar com material gráfico como impressão, digitalização e fotocópia. Minhas conquistas na empresa foram agilizar o atendimento aos advogados, implementar um script para limpar e desligar as máquinas via rede (a empresa não tinha software para isso e até então era tudo feito manualmente) onde nas 70 máquinas que existiam na empresa que era utilizada pelos advogados durante o dia, tiveram seus arquivos apagados, e a máquina desligada no final do dia. Outra conquista foi buscar conhecer sistemas judiciários que não pertenciam a Goiás, pois não havia locais para auxiliar advogados que pudessem ajudá-los aqui em Goiás.",
      ],
    },
    {
      title: "O que não gosto em um emprego!",
      text: [
        "Bem, em todo o meu trabalho, a única coisa que me deixou triste foi a falta de meritocracia. Acredito que o que torna um trabalho justo é a meritocracia. Alguém que é bem visto por seu esforço e trabalho duro, não por ser indicado por uma pessoa de alto escalão.",
      ],
    },
    {
      title: "O que é o melhor trabalho para mim!",
      text: [
        "O que acredito ser um trabalho de qualidade é justamente um trabalho onde a empresa usa a meritocracia como ideia principal para tratar seus funcionários.",
      ],
    },
  ],
};

export default myself;
